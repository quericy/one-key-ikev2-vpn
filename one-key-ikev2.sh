...
function get_key(){
    ...
    if [ ${interactive} -ne 0 ]; then
        echo "configure the root ca pkcs12 cert password(Can be empty):"
        openssl pkcs12 -export -inkey ca.key.pem -in ca.cert.pem -certfile ca.cert.pem \
                -out ca.cert.p12
    else
        echo "configure the root ca pkcs12 cert."
        openssl pkcs12 -export -inkey ca.key.pem -in ca.cert.pem -certfile ca.cert.pem \
                -out ca.cert.p12 -passout pass:${cert_password}
    fi
...
function generate_mobileconfig() {
    ...
    ca_pfx_base64=`base64 ${vpn_key_folder}/ca.cert.p12`
    #my_cert_cn
    #cert_password
